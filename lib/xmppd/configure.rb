...
class ConfigParser < Configure::Parser
    ...
    def handle_db(entry)
     ...
     entry.entries.each do |node|
         ...
         # basically what the file is called  
         when 'db_adaptor'
             db_config[:db_adaptor] = node.data
 
         # the class name. this will then be loaded as an adaptor in DB
         when 'class_name'
             db_config[:class_name] = node.data
     ...
     # require the file
     # xmppd/db-adaptors/class_name.downcase .rb

     # check the class as stated is defined

     # defined? DB.const_get(db.config.class_name)

     # if nothing is there, warn and default to YAML
